...
st.set_page_config(
    page_title="OFFLINE MCP Terminal Client",
    page_icon="⚡",
    layout="wide",
...
.sb-logo-text .name {
    ...
    color: var(--text0); letter-spacing: -0.3px; line-height: 1;
}
.sb-logo-text .tag {
...
/* ── STATUS PILL ── */
.sb-status-wrap { padding: 12px 20px; border-bottom: 1px solid var(--border0); }
.sb-status-pill {
...
.sb-status-pill.online  { background: rgba(200,255,87,0.08); border: 1px solid rgba(200,255,87,0.25); color: var(--acid); }
.sb-status-pill.connecting { background: rgba(255,170,0,0.08); border: 1px solid rgba(255,170,0,0.25); color: #ffaa00; }
.sb-status-pill.offline { background: rgba(255,70,70,0.08);  border: 1px solid rgba(255,70,70,0.25);  color: #ff4646; }
...
.sb-section-label::after {
    content: ''; flex: 1; height: 1px; background: var(--border0);
}
...
.sb-card:hover { background: var(--bg4); border-color: var(--border1); transform: translateX(4px); }
.sb-card:hover::before { background: var(--acid); }
.sb-card-top { display: flex; align-items: center; gap: 8px; margin-bottom: 4px; }
.sb-card-icon { font-size: 14px; width: 22px; text-align: center; flex-shrink: 0; }
.sb-card-name {
    font-family: 'JetBrains Mono', monospace;
...
.sb-card-desc {
    ...
    line-height: 1.55; padding-left: 30px;
}
.sb-cat {
    margin: 6px 16px 4px;
    font-family: 'JetBrains Mono', monospace;
    ...
    letter-spacing: 1.5px; text-transform: uppercase;
    color: var(--text2);
    padding-bottom: 4px; border-bottom: 1px dashed var(--border0);
}
...
section[data-testid="stSidebar"] .streamlit-expanderHeader {
    ...
    font-family: 'JetBrains Mono', monospace !important;
    font-size: 11px !important; font-weight: 500 !important;
    color: var(--text1) !important; letter-spacing: 0.3px !important;
...
section[data-testid="stSidebar"] .streamlit-expanderContent {
    border: none !important; padding: 4px 0 8px !important;
}

/* ── SIDEBAR STATS ── */
.sb-stats { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; padding: 0 16px 16px; }
.sb-stat { background: var(--bg3); border: 1px solid var(--border0); border-radius: var(--r-sm); padding: 10px 12px; }
.sb-stat-val { font-family: 'Inter', sans-serif; font-size: 22px; font-weight: 800; color: var(--text0); line-height: 1; }
.sb-stat-label { font-family: 'JetBrains Mono', monospace; font-size: 9px; color: var(--text2); text-transform: uppercase; letter-spacing: 1px; margin-top: 4px; }

/* ── SIDEBAR ALERTS ── */
section[data-testid="stSidebar"] .stSuccess,
section[data-testid="stSidebar"] .stSuccess > div {
    background: rgba(200,255,87,0.08) !important; border: 1px solid rgba(200,255,87,0.25) !important;
    color: var(--acid) !important; border-radius: var(--r-sm) !important;
}
section[data-testid="stSidebar"] .stWarning,
section[data-testid="stSidebar"] .stWarning > div {
    background: rgba(255,170,0,0.08) !important; border: 1px solid rgba(255,170,0,0.25) !important;
    color: #ffaa00 !important; border-radius: var(--r-sm) !important;
}
section[data-testid="stSidebar"] .stError,
section[data-testid="stSidebar"] .stError > div {
    background: rgba(255,70,70,0.08) !important; border: 1px solid rgba(255,70,70,0.25) !important;
    color: #ff4646 !important; border-radius: var(--r-sm) !important;
}
section[data-testid="stSidebar"] .stDownloadButton > button {
    ...
    font-size: 11px !important; padding: 9px 12px !important; transform: none !important;
}
section[data-testid="stSidebar"] .stDownloadButton > button:hover {
    background: var(--acid-glow) !important; border-color: var(--acid-border) !important; color: var(--acid) !important;
}
...
.main-hero {
    ...
    overflow: hidden; background: var(--bg1); border: 1px solid var(--border1);
}
.main-hero::before {
    content: ''; position: absolute;
    top: -60px; left: -40px; width: 340px; height: 260px;
    background: radial-gradient(ellipse, rgba(200,255,87,0.12) 0%, transparent 70%);
    pointer-events: none;
}
.main-hero::after {
    content: ''; position: absolute;
    bottom: -40px; right: -40px; width: 280px; height: 220px;
...
.hero-badge {
    ...
    padding: 5px 13px; border-radius: 50px; letter-spacing: 0.5px;
}
.badge-gold { background: rgba(255,193,7,0.12); border: 1px solid rgba(255,193,7,0.35); color: #ffc107; }
.badge-cyan { background: var(--cyan-glow); border: 1px solid rgba(0,229,255,0.25); color: var(--cyan); }
.badge-acid { background: var(--acid-glow); border: 1px solid var(--acid-border); color: var(--acid); }
...
.h-dot-online    { background: var(--acid); box-shadow: 0 0 14px rgba(200,255,87,0.7); animation: pDot 2s infinite; }
.h-dot-connecting{ background: #ffaa00;      box-shadow: 0 0 14px rgba(255,170,0,0.7);  animation: pDot 0.9s infinite; }
.h-dot-offline   { background: #ff4646;      box-shadow: 0 0 14px rgba(255,70,70,0.6); }
@keyframes pDot {
    0%,100% { transform:scale(1); opacity:1; }
    50%     { transform:scale(1.35); opacity:0.6; }
}
.h-text-online    { color: var(--acid); }
.h-text-connecting{ color: #ffaa00; }
.h-text-offline   { color: #ff4646; }
.h-uptime { font-size: 10px; color: var(--text2); font-weight: 500; letter-spacing: 0.3px; }
...
.stat-icon-pro { font-size: 26px; display: block; margin-bottom: 12px; filter: drop-shadow(0 0 10px rgba(200,255,87,0.3)); }
.stat-label-pro { font-family: 'JetBrains Mono', monospace; font-size: 9px; font-weight: 600; text-transform: uppercase; letter-spacing: 2px; color: var(--text2); display: block; margin-bottom: 6px; }
.stat-value-pro { font-family: 'Inter', sans-serif; font-size: 36px; font-weight: 800; color: var(--text0); display: block; line-height: 1; letter-spacing: -1px; }
.stat-sublabel-pro { font-family: 'Inter', sans-serif; font-size: 11px; color: var(--text2); display: block; margin-top: 6px; }
...
.empty-state {
    display: flex; flex-direction: column; align-items: center;
    justify-content: center; height: 360px; gap: 16px; text-align: center;
}
.empty-icon { font-size: 56px; animation: floatEmoji 3.5s ease-in-out infinite; filter: drop-shadow(0 0 18px rgba(200,255,87,0.4)); }
@keyframes floatEmoji {
    0%,100% { transform: translateY(0); }
    50%      { transform: translateY(-12px); }
}
.empty-title { font-family: 'Inter', sans-serif; font-size: 22px; font-weight: 800; color: var(--text0); letter-spacing: -0.3px; }
.empty-sub   { font-family: 'Inter', sans-serif; font-size: 14px; color: var(--text2); max-width: 360px; line-height: 1.7; }
.empty-chips { display: flex; gap: 8px; flex-wrap: wrap; justify-content: center; margin-top: 6px; }
.chip { font-family: 'JetBrains Mono', monospace; font-size: 11px; padding: 6px 14px; border-radius: 50px; background: var(--bg3); border: 1px solid var(--border1); color: var(--text1); }

/* ── MESSAGES ── */
.message-wrapper-pro { margin-bottom: 20px; animation: msgIn 0.25s ease-out; }
@keyframes msgIn {
    from { opacity:0; transform:translateY(6px); }
    to   { opacity:1; transform:translateY(0); }
}
.message-header { display: flex; align-items: center; gap: 8px; margin-bottom: 8px; }
.message-avatar { width: 26px; height: 26px; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 13px; flex-shrink: 0; }
...
.msg-role { font-family: 'JetBrains Mono', monospace; font-size: 11px; font-weight: 500; color: var(--text2); text-transform: uppercase; letter-spacing: 0.8px; }
.msg-time { font-family: 'JetBrains Mono', monospace; font-size: 10px; color: var(--text2); margin-left: auto; }
.message-user-pro { color: var(--text0); word-wrap: break-word; line-height: 1.65; font-size: 14px; }
.message-user-pro .msg-body { background: rgba(200,255,87,0.05); border: 1px solid rgba(200,255,87,0.14); border-left: 3px solid var(--acid); border-radius: 0 10px 10px 10px; padding: 13px 16px; }
.message-assistant-pro { color: var(--text0); word-wrap: break-word; line-height: 1.7; font-size: 14px; }
.message-assistant-pro .msg-body { background: var(--bg3); border: 1px solid var(--border0); border-left: 3px solid var(--cyan); border-radius: 0 10px 10px 10px; padding: 14px 18px; }

/* ── INPUT ── */
.input-container-pro {
    background: var(--bg1); border: 1px solid var(--border1);
    border-radius: var(--r-lg); padding: 18px 22px; transition: border-color 0.2s;
}
...
.stTextInput > div > div > input {
    background: var(--bg2) !important; border: 1px solid var(--border1) !important;
...
/* ── MAIN BUTTONS ── */
.stButton > button {
    background: var(--acid) !important; color: #05080f !important;
    border: none !important; border-radius: var(--r-sm) !important;
    padding: 14px 22px !important; font-family: 'Inter', sans-serif !important;
...
.stButton > button:hover { transform: translateY(-2px) !important; box-shadow: 0 10px 32px rgba(200,255,87,0.55) !important; }
.stButton > button:active { transform: translateY(0) !important; }

/* ── SHORTCUTS ── */
.shortcuts-bar { text-align: center; margin-top: 12px; display: flex; align-items: center; justify-content: center; gap: 18px; flex-wrap: wrap; }
.shortcut { display: flex; align-items: center; gap: 5px; font-family: 'JetBrains Mono', monospace; font-size: 10px; color: var(--text2); }
kbd { background: var(--bg3); border: 1px solid var(--border1); border-radius: 5px; padding: 2px 7px; font-family: 'JetBrains Mono', monospace; font-size: 10px; color: var(--text1); }
...
def render_message_pro(message: Dict[str, Any]):
    """Render a single chat message. Content is HTML-escaped to prevent injection."""
    ...
    if message["role"] == "user":
        st.markdown(
            f'<div class="message-wrapper-pro">'
            f'<div class="message-user-pro">'
            f'<div class="message-header">'
            f'<div class="message-avatar avatar-user">👤</div>'
        ...
        st.markdown(
            f'<div class="message-wrapper-pro">'
            f'<div class="message-assistant-pro">'
            f'<div class="message-header">'
            f'<div class="message-avatar avatar-assistant">🤖</div>'
...
def build_hero_html(manager) -> str:
    """
    Build the complete hero HTML as a single string.
    This avoids the nested f-string interpolation bug where Streamlit
    escapes the inner status HTML when it's built separately.
    """
    # Status dot + label
    if manager.connected:
        dot_cls  = "h-dot h-dot-online"
        txt_cls  = "h-text-online"
        txt      = "ONLINE"
        uptime   = f'<div class="h-uptime">⏱ {manager.get_uptime()} uptime</div>'
    elif manager.connecting:
        dot_cls  = "h-dot h-dot-connecting"
        txt_cls  = "h-text-connecting"
        txt      = "CONNECTING"
        uptime   = ""
    else:
        dot_cls  = "h-dot h-dot-offline"
        txt_cls  = "h-text-offline"
        txt      = "OFFLINE"
        uptime   = ""
    ...
    return (
        ...
        f'<div class="h-status-row">'
        f'<span class="{dot_cls}"></span>'
        f'<span class="{txt_cls}">{txt}</span>'
        f'</div>'
        f'{uptime}'
        '</div>'
        '</div>'
...
def render_sidebar_pro(manager: UltimateMCPManager):
    with st.sidebar:
        ...
        if manager.connected:
            pill_cls = "online"
            label    = "Connected"
            sub      = f"Uptime: {manager.get_uptime()}"
        elif manager.connecting:
            pill_cls = "connecting"
            label    = "Connecting…"
            sub      = "Establishing MCP link"
        else:
            pill_cls = "offline"
            label    = "Offline"
        ...
        st.markdown(
            f'<div class="sb-status-wrap">'
            f'<div class="sb-status-pill {pill_cls}">'
            f'<span class="dot"></span>'
            f'<div class="pill-info">'
            f'<div class="pill-label">{label}</div>'
            f'<div class="pill-sub">{sub}</div>'
            f'</div></div></div>',
            unsafe_allow_html=True
        )
        ...
        if st.session_state.get('show_sessions', False):
            ...
            if sessions:
                ...
                st.markdown(
                    '<div style="padding:4px 0 10px;font-family:JetBrains Mono,monospace;'
                    'font-size:11px;color:var(--text2);">No sessions yet</div>',
                    unsafe_allow_html=True
                )
            ...
            if tools:
                for cat in sorted(tools_by_cat.keys()):
                    ...
                    for t in tools_by_cat[cat]:
                        desc = html.escape((t['description'] or '')[:72])
                        name = html.escape(t['name'])
                        st.markdown(
                            f'<div class="sb-card">'
                            f'<div class="sb-card-top">'
                            f'<span class="sb-card-icon">{icon}</span>'
                            f'<span class="sb-card-name">{name}</span>'
                            f'<span class="sb-card-badge badge-t">{cat}</span>'
                            f'</div>'
                            f'<div class="sb-card-desc">{desc}</div>'
                            f'</div>',
                            unsafe_allow_html=True
                        )
            else:
                st.markdown(
                    ...
                    'font-size:11px;color:var(--text2);">No tools available</div>',
                    unsafe_allow_html=True
                )
        ...
        with st.expander(f"📁 Resources  ·  {len(resources)}", expanded=False):
            if resources:
                for r in resources:
                    ...
                    icon = CAT_ICONS.get(cat, '📄')
                    desc = html.escape((r['description'] or '')[:72])
                    ...
                    st.markdown(
                        ...
                        f'<span class="sb-card-icon">{icon}</span>'
                        f'<span class="sb-card-name">{name}</span>'
                        f'<span class="sb-card-badge badge-r">{cat}</span>'
                        f'</div>'
                        f'<div class="sb-card-desc">{desc}</div>'
                        f'</div>',
                        unsafe_allow_html=True
                ...
                st.markdown(
                    '<div style="padding:8px 16px;font-family:JetBrains Mono,monospace;'
                    'font-size:11px;color:var(--text2);">No resources available</div>',
                    unsafe_allow_html=True
                )
        ...
        with st.expander(f"💬 Prompts  ·  {len(prompts)}", expanded=False):
            if prompts:
                for p in prompts:
                    desc = html.escape((p['description'] or '')[:72])
                    name = html.escape(p['name'])
                    st.markdown(
                        f'<div class="sb-card">'
                        f'<div class="sb-card-top">'
                        f'<span class="sb-card-icon">💬</span>'
                        f'<span class="sb-card-name">{name}</span>'
                        f'<span class="sb-card-badge badge-p">prompt</span>'
                        f'</div>'
                        f'<div class="sb-card-desc">{desc}</div>'
                        f'</div>',
                        unsafe_allow_html=True
                ...
                st.markdown(
                    '<div style="padding:8px 16px;font-family:JetBrains Mono,monospace;'
                    'font-size:11px;color:var(--text2);">No prompts available</div>',
                    unsafe_allow_html=True
                )
        ...
        st.markdown(
            ...
            f'<div class="sb-stat"><div class="sb-stat-val">{manager.stats["messages_sent"]}</div><div class="sb-stat-label">Messages</div></div>'
            f'<div class="sb-stat"><div class="sb-stat-val">{manager.stats["avg_response_time"]:.1f}s</div><div class="sb-stat-label">Avg resp.</div></div>'
            f'<div class="sb-stat"><div class="sb-stat-val">{sr}%</div><div class="sb-stat-label">Success</div></div>'
            f'<div class="sb-stat"><div class="sb-stat-val">{len(tools)}</div><div class="sb-stat-label">Tools</div></div>'
            f'</div>',
            unsafe_allow_html=True
        )
...
def main():
    ...
    # ── HERO — built as one string, no nested f-strings ──
    st.markdown(build_hero_html(manager), unsafe_allow_html=True)
    ...
    st.markdown(
        f'<div class="stats-grid-pro">'
        f'<div class="stat-card-pro"><span class="stat-icon-pro">💬</span><span class="stat-label-pro">Messages</span><span class="stat-value-pro">{len(st.session_state.messages)}</span><span class="stat-sublabel-pro">This session</span></div>'
        f'<div class="stat-card-pro"><span class="stat-icon-pro">🔧</span><span class="stat-label-pro">Tools</span><span class="stat-value-pro">{len(tools)}</span><span class="stat-sublabel-pro">Available</span></div>'
        f'<div class="stat-card-pro"><span class="stat-icon-pro">📁</span><span class="stat-label-pro">Resources</span><span class="stat-value-pro">{len(resources)}</span><span class="stat-sublabel-pro">Accessible</span></div>'
        f'<div class="stat-card-pro"><span class="stat-icon-pro">⚡</span><span class="stat-label-pro">Response</span><span class="stat-value-pro">{manager.stats["avg_response_time"]:.1f}s</span><span class="stat-sublabel-pro">Average time</span></div>'
        f'<div class="stat-card-pro"><span class="stat-icon-pro">✅</span><span class="stat-label-pro">Success</span><span class="stat-value-pro">{success_rate:.0f}%</span><span class="stat-sublabel-pro">Success rate</span></div>'
        f'</div>',
        unsafe_allow_html=True
    )
    ...
    if not st.session_state.messages:
        st.markdown(
            ...
            '<div class="empty-sub">AI-powered MCP terminal. Type any command or question to begin.</div>'
    ...
    sid_display = html.escape(st.session_state.current_session)
    st.markdown(
        f'<div class="shortcuts-bar">'
        f'<span class="shortcut"><kbd>Enter</kbd> Send</span>'
        f'<span class="shortcut"><kbd>Ctrl+K</kbd> Clear</span>'
        f'<span class="shortcut"><kbd>↑↓</kbd> History</span>'
        f'<span class="shortcut" style="font-family:JetBrains Mono,monospace;font-size:10px;color:var(--text2);">'
        f'session <span style="color:rgba(200,255,87,0.6);">{sid_display}</span>'
        f'</span></div>',
        unsafe_allow_html=True
    )
